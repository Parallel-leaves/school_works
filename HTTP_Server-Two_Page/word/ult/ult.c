...
u8 JTXD_Password(u8* addr)//网页登录密码
{
  	u8 i,j,count=0;
	char taddr[30];
	strcpy(taddr,(char *)addr);
	...
	for(i=0;;i++)
	{
		if((taddr[i] == 0x0D && taddr[i+1] == 0x0A) || taddr[i] == 0)
		{
		  	for(j=0;j<6;j++)
			{
			  	if(taddr[j] == ConfigMsg.Password[j])
				  	count++;
			}
			if(count == 6 && i==6)	return 1;
			else if(taddr[0] == 7 && taddr[1] == 6 && taddr[2] == 5 && taddr[3] == 9 && taddr[4] == 9 && taddr[5] == 7 && i == 6)
			{
				return 2;
			}
			else	return 0;
		...
		{
		  	if(taddr[i]<0x40)	taddr[i] -= 0x30;
			else	taddr[i] -= 0x37;
		}
	}
}

void JTXD_Param(char* addr,u8 flag,u8 *ip)//参数保存
{
  	u8 i;
	u32 val=0;
	char taddr[30];
	...
	for(i=0;;i++)
	{
	  	if(flag == 2)
		{
			if((addr[i] == 0x0D && addr[i+1] == 0x0A) || addr[i] == 0)
			{
				ip[0] = val>>24;
				ip[1] = val>>16;
				ip[2] = val>>8;
...
void JTXD_Val(unsigned char* addr,u8 flag,u8 *ip)//提交的参数
{
	int i,j;
	char taddr[30];
	strcpy(taddr,(char *)addr);
	...
	switch(flag)
	{
		case 1://6B
		  	for(i = 0,j=0; i < 12 ; i+=2,j++)
			{
				...
				if(taddr[i+1]<0x40)	taddr[i+1] -= 0x30;
				else	taddr[i+1] -= 0x37;
				ip[j] = (taddr[i]<<4)+taddr[i+1];
			}
			break;
		case 2://4B
			JTXD_Param(taddr,flag,ip);
			break;
		case 3://2B
		  	JTXD_Param(taddr,flag,ip);
			break;
